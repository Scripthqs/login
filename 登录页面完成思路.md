...
**定义一个check函数，验证账号和密码，使用span标签配合v-if显示提示消息，当定时器2s后或者监听input输入事件时，让v-if=false隐藏提示消息。**
...
为了防止用户连续多次点击按钮，可以考虑使用防抖，或者设置定时器，让按钮在点击1次后，5s内不可再次点击
...
- 基本功能已经完成，但是if判断语句过多，应该还存在优化的空间。
- 定时器过多
- 输入框还可以考虑加入清除功能。
- 密码框还可以考虑点击小眼睛显示隐藏密码。
- 提示框可以考虑封装成组件。
